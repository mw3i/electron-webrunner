var edgeObservation2 = {
    start: function(params, next) {
        ...
        var s = {
            ...
            state: 'observation',
            finish_early: false,
            block_order: edgeObservation2.generateBlockOrder(params['pairs']),
            connection: null,
        }

        s['connection'] = s['block_order'][s['trial']][2]
        elements = edgeObservation2.initElements()
        edgeObservation2.initStim(
        ...
        function next_trial() {
            ...
            if (s['trial'] >= s['block_order'].length) {
                ...
                if (s['block'] >= params['n_blocks']) {
                    edgeObservation2.destroySpace()
                    next()
                }
                s['block_order'] = edgeObservation2.generateBlockOrder(params['pairs'])
            }
    ...
    generateBlockOrder: function (obj_set) {
        obs7 = obj_set[0]
        obs8 = obj_set[1]
        obs9 = obj_set[2]

        idx = [...Array(8).keys()] // from ben @ https://stackoverflow.com/a/10050831
        idx = edgeObservation2.shuffle(idx)
        obs8.push(obs8[idx[0]])

        idx = [...Array(7).keys()] // from ben @ https://stackoverflow.com/a/10050831
        idx = edgeObservation2.shuffle(idx)
        obs9.push(obs9[idx[0]])
        obs9.push(obs9[idx[1]])

        objects = [...obs7, ...obs8, ...obs9]
        ...
        for (let i of idx) {
            block.push([
                ...edgeObservation2.shuffle([objects[i][0], objects[i][1]]),
                objects[i][2]
            ])
        }
    ...
    mean: function(numbers) {
        return edgeObservation2.sum(numbers) / numbers.length;
    },
